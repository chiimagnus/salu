...
> 说明：本文档描述的是“业务规则与结算口径”，不要求各客户端（CLI/AVP）在同一时间点完成全部 UI/便利功能；例如 AVP 当前阶段刻意不实现存档/读档/Continue 与回放工具。
...
| 灵视 | — | 0 | 预知 2，+1 疯狂 | 闭上双眼，第三只眼便会睁开 |
| 真相低语 | — | 1 | 造成 5 伤害，预知 1，+1 疯狂 | 真相是最锋利的刀刃 |
| 冥想 | — | 1 | 获得 4 格挡，清除 2 疯狂 | 片刻宁静弥足珍贵 |
| 理智燃烧 | — | 1 | 获得 2 力量，+3 疯狂 | 燃烧理智，换取力量 |
| 命运改写 | — | 1 | 改写：目标敌人意图变为“防御”，+2 疯狂 | 命运的丝线在我指尖缠绕 |
...
- **奖励 / 商店 / 事件可获得卡池（非起始卡）**：通用卡池 + 占卜家基础卡（灵视、真相低语、冥想、理智燃烧、命运改写）等（以代码注册表为准）
...
- **触发：进入商店房间**
  - 商店提供 **5 张可购买卡牌**（来自非起始卡池）
  - 商店提供 **3 个可购买遗物**（不会出现已拥有遗物）
  - 商店提供 **3 张可购买消耗性卡牌**（原“消耗品”，`CardType.consumable`）
...
- **购买遗物**：
  - 金币足够时才能购买
  - 购买后遗物加入遗物栏，商店对应遗物从库存移除
- **购买消耗性卡牌**：
  - 金币足够时才能购买
  - 购买后占用消耗性卡牌槽位（最多 3 个）；**槽位满时不可购买**
- **删牌服务**：
  - 金币足够时才能删牌
  - 选择牌组中任意 1 张牌移除
...
- 输入 `1-5`：购买对应编号的卡牌
- 输入 `R1-R3`：购买对应编号的遗物
- 输入 `C1-C3`：购买对应编号的消耗性卡牌
- 输入 `D`：进入删牌服务
- 输入 `0`：离开商店

### 8.5 消耗性卡牌系统（v1.0，原“消耗品”）

- **形态**：它们本质上是牌组里的卡牌实例（`CardType.consumable`），会像普通卡牌一样进入战斗牌堆并被抽到。
- **费用规则**：费用恒为 0，打出时不消耗能量。
- **一次性规则**：本战斗打出后进入 Exhaust；战斗结算时会从冒险牌组（`RunState.deck`）永久移除，后续战斗不再出现。
- **槽位上限**：最多持有 3 张消耗性卡牌（槽位满时不可购买新的）。
- **当前已实现的消耗性卡牌**：
  - 治疗药剂：恢复 20 HP（战斗内，通过“抽到并打出”使用）
  - 格挡药剂：获得 12 格挡（战斗内）
  - 力量药剂：获得 2 力量（战斗内）
  - 净化符文（占卜家）：清除所有疯狂（战斗内）
...
- **触发：进入事件房间**
  - 展示事件名称与描述（v1.0：以剧情事件/NPC 对话/世界观碎片为主）
  - 展示多个可选项（通常包含“离开”）
...
| 序列密室 📚 | 阅读禁书 / 焚毁书页 / 离开 | 获得卡牌；疯狂变化；失去 HP |
| 时间裂隙 ⏳ | 窥视过去 / 窥视未来 / 闭眼离开 | 升级卡牌（二次选择）；获得遗物；回复 HP；疯狂变化 |
| 疯狂预言者 🔮 | 聆听预言 / 打断他 / 给予金币安抚 | 获得稀有卡；进入精英战；回复 HP；疯狂变化 |
...
| 理智之锚 | — | 所有疯狂阈值 +3 | 被动 | 抓住这根锚链——它是你最后的理智 |
| 深渊之瞳 | — | 战斗开始时预知 3，+1 疯狂 | 战斗开始 | 深渊赠予你洞察——代价是它也在注视你 |
| 预言者手札 | — | 每场战斗首次使用改写时，不获得疯狂 | 改写触发（每场战斗限 1 次） | 前人的智慧刻在纸上——墨迹下藏着血泪 |
| 疯狂面具 | — | 当疯狂 ≥6 时，攻击伤害 +50% | 被动（条件触发） | 戴上它，你会失去理智——也会获得力量 |

### 11.4 遗物池（当前版本）

- **起始遗物**：永燃心脏
- **可掉落遗物（精英/Boss）**：远古骨锤、幽冥灯火、鳞甲残片、夜鸦羽翼、血誓旗帜、始祖碎片、第三只眼、破碎怀表、理智之锚、深渊之瞳、预言者手札、疯狂面具
...
- **版本策略**：存档格式为**破坏性变更策略**（版本不兼容则拒绝加载），请以当前代码实现为准。
