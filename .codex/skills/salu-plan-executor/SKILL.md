...
- 用户指定一个 plan 文件路径（通常在 `.cursor/plans/*.md`）。
...
- 如行为有变化：同步更新 `README.md` 或对应 `.cursor/rules/*.mdc`（仅在必要时）。
