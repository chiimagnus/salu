...
| 步骤 | 内容 | 复杂度 | 预计时间 |
|------|------|--------|----------|
| P1.1 | 添加第二个敌人（信徒），硬编码属性 | ⭐ | 10分钟 |
| P1.2 | 创建 `EnemyKind` 枚举，随机选择敌人 | ⭐ | 15分钟 |
| P1.3 | 敌人不同攻击值（从 Entity 属性读取） | ⭐ | 15分钟 |
| P1.4 | 敌人意图系统（攻击/防御/增益） | ⭐⭐ | 30分钟 |
| P1.5 | 敌人 AI 决策（根据状态选择行动） | ⭐⭐⭐ | 1小时 |
| P1.6 | 添加 3-5 种敌人，测试平衡 | ⭐⭐ | 1小时 |

**验收标准**：游戏随机出现不同敌人，敌人有智能行为
...
| 步骤 | 内容 | 复杂度 | 预计时间 |
|------|------|--------|----------|
| P2.1 | 创建线性地图（固定 5 个战斗节点） | ⭐ | 20分钟 |
| P2.2 | 添加地图界面，显示当前位置 | ⭐ | 30分钟 |
| P2.3 | 连续战斗（生命值保持） | ⭐ | 20分钟 |
| P2.4 | 添加休息节点（恢复生命） | ⭐ | 20分钟 |
| P2.5 | 分叉路径（玩家可选择） | ⭐⭐ | 45分钟 |
| P2.6 | 程序生成完整地图 | ⭐⭐⭐ | 1.5小时 |
| P2.7 | Boss 节点 | ⭐⭐ | 30分钟 |

**验收标准**：游戏有完整的一层地图，玩家可选择路径
