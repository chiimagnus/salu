...
- [x] 普通战斗胜利后出现奖励界面（可跳过）
- [x] 选择卡牌后 deck +1，存档后继续冒险 deck 仍保留
- [x] 固定 seed 下奖励可复现（同一路径）
- [x] `./.cursor/Scripts/test_game.sh all` 通过

## 6. 可选优化（不阻塞 P1 验收）

> 这些优化建议在 P2（金币系统）落地后更自然。

- **稀有度权重**：基于 `CardDefinition.rarity` 为奖励抽取增加权重（目前卡池主要是 common）
- **避免重复**：同一次 offer 内保证不重复（已满足）；进一步可以按 deck 里已有卡限制重复出现频率
- **跳过给金币**：跳过奖励时给少量金币（需要先引入 `RunState.gold`）

### 6.1 可执行任务清单（建议作为 P1.1）

- [ ] **按稀有度权重抽取**
  - [ ] 设计权重表（common/uncommon/rare）
  - [ ] `RewardGenerator` 支持按权重抽取（仍保证去重与可复现）
  - [ ] 为权重引入最小可测性：固定 seed 下输出稳定
- [ ] **降低重复出现频率**
  - [ ] 同一 offer 内已去重（保持）
  - [ ] 进一步：当 deck 已包含某 CardID 时，降低其再次出现概率（或直接在 offer 中排除）
- [ ] **跳过奖励给金币（依赖 P2）**
  - [ ] `RunState` 引入 `gold`
  - [ ] skip 时 `runState.gold += X`
  - [ ] 测试覆盖：reward test + shop test（P2）
