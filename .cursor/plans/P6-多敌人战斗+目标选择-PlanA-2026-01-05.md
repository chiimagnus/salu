...
**验收**
- 实机普通战斗可遇到 2 敌人同场，并能通过 `卡牌序号 目标序号` 选目标击杀其中一个，战斗继续直到全部敌人死亡。
- 为了稳定验收（不依赖概率），可使用 `SALU_FORCE_MULTI_ENEMY=1` 强制普通战斗进入双敌人遭遇；固定 `--seed` 可复现同一次冒险过程。
...
- `Tests/GameCLIUITests/*`
  ...
  - 新增：`GameCLIMultiEnemyTargetingUITests`（`SALU_TEST_MAP=battle` + `SALU_FORCE_MULTI_ENEMY=1`）覆盖“双敌人 + 攻击牌必须选目标”的黑盒路径
