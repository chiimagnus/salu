...
**状态（已执行）**
- ✅ 完成（2026-01-06）：补充 Act2/遗物/资源页/多幕推进的确定性与流程测试；并新增 1 个 Act 推进 UI 黑盒测试
- ✅ 验证：`swift test` / `swift build -c release` 通过

**新增/更新测试（摘要）**
- `Tests/GameCoreTests/EnemyDeterminismTests.swift`：Act2 敌人池/遭遇池可复现；Act2 敌人 chooseMove 可复现
- `Tests/GameCoreTests/EnemyPoolAndBasicEnemiesTests.swift`：Act2 敌人池 pick；Act2 敌人/Boss 行为分支覆盖
- `Tests/GameCoreTests/RegistrySmokeTests.swift`：Act2 Pool + Boss 必须注册
- `Tests/GameCoreTests/RelicManagerAndRelicsTests.swift`：新增遗物触发测试（铁护臂/羽披风/战旗/巨像核心）
- `Tests/GameCLIUITests/GameCLIMultiActProgressionUITests.swift`：Act1 Boss 后进入 Act2 并持久化存档
- `Tests/GameCLITests/ScreenAndRoomCoverageTests.swift`：资源页覆盖新增 Act2 断言
