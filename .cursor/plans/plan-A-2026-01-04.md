...
- 测试：`./.cursor/Scripts/test_game.sh all` 可通过
...
4. 测试：
   - 新增 `test_reward.sh`：固定 seed，跑一场战斗后能进入奖励界面；选择后 deck 数量变化可观察
...
4. 测试：
   - `test_shop.sh`：固定 seed，能进入商店并完成一次购买（金币减少，deck 增加）
...
3. 测试：
   - `test_upgrade.sh`：固定 seed，进入休息点升级后 deck 中存在 `strike+` 等
...
3. 测试：
   - `test_relic_reward.sh`：固定 seed，精英胜利后能拿到 relic，后续 battleStart 触发生效（可通过事件/能量变化观察）
...
3. 测试：`test_event.sh`（固定 seed，事件选择后 RunState 变化可观察）
