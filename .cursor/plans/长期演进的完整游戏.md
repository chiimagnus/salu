# Plan A（P1/P2/P3...）：先把 GameCore + GameCLI 做到“可长期演进的完整游戏”，再启动 macOS SwiftUI App - 2026-01-10

## 0. 目标（你最新确认的方向）

- **设定方向（v0.1）**：卡牌 Roguelike + 碎片叙事；克苏鲁 / 神秘学风格；通关即结局。事实来源：`.cursor/plans/Salu 游戏设计方案（v0 1） .md`
...
- `GameCLI`：
  ...
  - 叙事文案/命名以 v0.1 为准：优先通过 Registry 的展示字段（name/rulesText/description 等）替换“旧占位名”，避免为“换皮”改动战斗规则
...
- **M0：可通关版**：至少 2~3 Act、完整房间链路、存档/继续、结算与统计；并具备“叙事闭环”（开场 → 终章结局）
- **M1：1.0 内容版**：显著扩充卡/敌人/事件/遗物 + 平衡调优 + 难度阶梯（Ascension/挑战）；叙事内容可持续扩充（更多剧情事件/NPC 互动/碎片收集）
...
- 冒险主线可稳定通关（建议目标：**3 Act**；与 v0.1 的“三章故事线：觉醒/真相/终结”对齐；若当前先 2 Act，也要把扩到 3 Act 的接口留好）
- 节点完成自动存档、主菜单继续、冒险结束清档稳定可靠
- 战斗/奖励/遗物/事件/商店/休息点链路无硬崩
- 叙事 MVP 闭环（改动最小）：
  - 事件房间：以“剧情事件/NPC 对话/世界观碎片”为主（不要求新增系统，只要求文案与选项足够承载叙事）
  - 休息点：以“据点/NPC 聊天”为主题包装，保留回血/升级结算
  - Boss 战后：补“章节收束文本/结局文本”
- 第一批重命名落地（先换展示名，不动规则）：卡牌/敌人/遗物采用 v0.1 命名映射（见 `.cursor/plans/Salu 游戏设计方案（v0 1） .md`）
...
- 事件池扩充（风险/收益更丰富）
- 事件池扩充（叙事要求）：覆盖“主线碎片 + 可重复支线”，并支持后续在 run 之间逐步展开（Hades 式增量叙事）
- 遗物池扩充（Build 方向更明确）
