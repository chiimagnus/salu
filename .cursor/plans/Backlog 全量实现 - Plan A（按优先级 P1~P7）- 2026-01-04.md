...
- **2026-01-05**：对 **P2/P3/P4** 做一次实现审计并补齐关键缺口（金币发放、地图保底商店/休息点、战斗界面遗物展示、奖励界面金币展示）；已验证 `swift test` + `swift build -c release`。
...
- 补上“战斗胜利给金币”的缺口（普通/精英胜利都会发放金币，且可复现）
- 地图生成增加“至少 1 个商店节点”的保底规则（避免极端 seed 整张图无商店）
- 战斗奖励界面显示“本场获得金币”（仅 UI 展示，不扩散到事件日志/结算页）
...
- 地图生成增加“第 6/12 层至少有 1 个休息点”的保底规则（更贴近“固定有休息点”的设定）
...
- 战斗界面补充显示“已持有遗物”（至少 icon + 名称），与地图界面保持一致
